...
const reset = async (mail) => {
    const RespUser = await fetch(`https://backend-16d8.onrender.com/api/usuarios/confirmUser/${mail}`, {
        method: "GET"
    });
    ...
    //Primer chequeo
    const isValidResp = await fetch(`https://backend-16d8.onrender.com/api/usuarios/isValidPassword/${mail}/${old_pass}`, {
        method: "GET"
    });
    ...
    let password_hashed_resp =  await fetch(`https://backend-16d8.onrender.com/api/usuarios/hashPassword/${new_pass}`, {
        method: "GET"
    });
    let password_hashed = await password_hashed_resp.text();
    console.log("password-hashed es: ", password_hashed);
    //actualizar usuario con nueva contraseña
    const updateResp = await fetch(`https://backend-16d8.onrender.com/api/usuarios/updateUser/${mail}`, {
        method: "PUT",
        headers: {
